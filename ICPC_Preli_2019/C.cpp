...
struct st
{
    int id,tot,time,af,sum;
} ss[55];


vector<int> vt[57];
int test, num,x,arr1[55],arr2[55],xx=0,nxts,val1,val2,val3,val4,nxtt,vis,cas=0;
int main()
{
...
//    freopen("output.txt", "w", stdout);
    ...
    while(test--)
    {
        ...
        nxts=ss[arr1[0]].tot+ss[arr1[0]].af;
        nxtt=ss[arr1[0]].time+ss[arr1[0]].sum;
       // cout<<nxts<<" "<<nxtt<<endl;
        FOR(i,1,num)
        {
            if(nxts>(ss[arr1[i]].tot+ss[arr1[i]].af))
            {
                nxts=ss[arr1[i]].tot+ss[arr1[i]].af;
                nxtt=ss[arr1[i]].time+ss[arr1[i]].sum;
                    //    cout<<nxts<<" 1"<<nxtt<<endl;

            }
            else if(nxts==(ss[arr1[i]].tot+ss[arr1[i]].af))
            {
                if( nxtt<=(ss[arr1[i]].time+ss[arr1[i]].sum))
                {
                    nxts=ss[arr1[i]].tot+ss[arr1[i]].af;
                    nxtt=ss[arr1[i]].time+ss[arr1[i]].sum;
                          //  cout<<nxts<<" 2 "<<nxtt<<endl;
                ...
                else
                {
                    ...
                    {
                        nxts--;
                        int siz=vt[arr1[i]].size();
                        nxtt=ss[arr1[i]].time+ss[arr1[i]].sum-vt[arr1[i]][siz-1];
                                                    cout<<nxts<<" 3 "<<nxtt<<endl;
            ...
            {
                                        //    cout<<nxts<<" 4 "<<nxtt<<endl;

                if ((nxts-ss[arr1[i]].tot)<=ss[arr1[i]].af)
                {
                                           //     cout<<nxts<<" 5 "<<nxtt<<endl;

                    int a=ss[arr1[i]].tot+ss[arr1[i]].af-nxts;
                    ...
                    for(int s=siz-1; s>=0; s--)
                    {
                        ...
                        c+=vt[arr1[i]][s];
                        a--;
                        if(nxts==(ss[arr1[i]].tot+a) && (nxtt<=(ss[arr1[i]].sum-c+ss[arr1[i]].time)))
                        {

                            nxtt=ss[arr1[i]].sum-c+ss[arr1[i]].time;
                            break;
                        }
                    }
                    nxtt=ss[arr1[i]].sum-c+ss[arr1[i]].time;
                    nxts=ss[arr1[i]].tot+a;
                }
                else
                {
                    vis=1;
