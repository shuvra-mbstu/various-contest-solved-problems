...
*/
#define Mx 90000000000
long long int test,row,col,x,y,arr[107][107],dp[107][107],x1=0,x2=0;
long long int fun(int ind1, int ind2)
{
    cout<<ind1<<" "<<ind2<<endl;
    long long int v1,v2;
    if(ind1+1==col && ind2+1==row)return 0;
 //  cout<<ind1<<" "<<ind2<<endl;
if(dp[ind1][ind2]!=-1)return dp[ind1][ind2];

if(ind1+1<col){
        int xx;
    if(abs(arr[ind1][ind2]-arr[ind1+1][ind2])%2)xx+=x;
     else xx=0;

        v1=fun(ind1+1,ind2)+xx;

}
 if(ind2+1<row){
        int xx;
    if(abs(arr[ind1][ind2]-arr[ind1][ind2+1])%2)xx+=x;
     else xx=0;

        v2=fun(ind1,ind2+1)+xx;
}
    return dp[ind1][ind2] =min(v2,v1);
}
long long int fun2(int ind1, int ind2)
{
   long long int v1,v2;
    if(ind1+1==col && ind2+1==row)return 0;
//    cout<<ind1<<" "<<ind2<<" "<<val1<<" "<<val2<<" "<<coun<<endl;
if(dp[ind1][ind2]!=-1)return dp[ind1][ind2];

if(ind1+1<col){
        int xx;
    if(!abs(arr[ind1][ind2]-arr[ind1+1][ind2])%2)xx+=y;
     else xx=0;

        v1=fun2(ind1+1,ind2)+xx;

}
 if(ind2+1<row){
        int xx;
    if(!abs(arr[ind1][ind2]-arr[ind1][ind2+1])%2)xx+=y;
     else xx=0;

        v2=fun2(ind1,ind2+1)+xx;
}
    return dp[ind1][ind2] =min(v2,v1);
}
int main()
