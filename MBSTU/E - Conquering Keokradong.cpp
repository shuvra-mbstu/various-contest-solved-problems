...
*/
long long int test,num,arr[1007],night,sum,cas=0;
//vector<int>anss(309),ans(309),a;
bool funn(int x)
{
    //    cout<<x<<" ASDEF ";
    int coun=0,ss=0;
    FOR(i,0,num+1)
    {
        ...
        {
            if(coun>(night) || arr[i]>x)
                return false;

            // cout<<ss<<" sdf "<<coun<<endl;
            coun++;
            ss=arr[i];
        }
    }
...
void path(int v,int x)
{
    int coun=0,sum=0;
    for(int i=0; i<=num; i++)
    {
        //  cout<<num<<" "<<coun<<" "<<x<<" "<<sum<<" "<<arr[i]<<endl;
        if(((sum+arr[i])<=v)&&((num+1-coun)>=x))
        {
            sum+=arr[i];
            coun++;
        }
...
{
    // cout<<" dfljbg \n";
    long long int lo=0, hi=10000000,mid,an,a;
    while(lo<hi)
    {
        mid=(lo+hi)/2;
        //   cout<<hi<<" "<<lo<<" "<<mid<<endl;
