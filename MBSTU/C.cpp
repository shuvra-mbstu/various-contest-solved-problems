...
struct st
{
    ll val,x=-1,y=-1,rng=-1;
} ss[3*Mx];
ll num,q,arr1[Mx],arr[Mx],v1,v2,v3,ans,anss,cas=0,val;
void build_tree(int node, int i,int j)
{
    ...
    int mid=(i+j)/2;
    build_tree(node*2,i,mid);
    build_tree(node*2+1,mid+1,j);
    //ss[node].mn=min(ss[node*2].mn,ss[node*2+1].mn);
    return;
}
void update(int node, int b, int e, int i, int j,int x,int y)
{

    if(ss[node].x!=-1)
    {
//cout<<"SDS "<<ss[node].pro<<" SDS"<<endl;
        if(e!=b)
        {
            ss[node*2].x=(ss[node].x);
            ss[node*2+1].x=(ss[node].x);
              ss[node*2].y=(ss[node].y);
            ss[node*2+1].y=(ss[node].y);
            ss[node*2].rng=(ss[node].rng);
            ss[node*2+1].rng=(ss[node].rng);
//cout<<"SDS "<<ss[node].mn<<" SDS"<<endl;
        }
        ss[node].x=-1;
    }
...
//cout<<"SDS "<<ss[node].mn<<" SDS"<<endl;
    ...
    int mid=(e+b)/2;
    update(node*2,b,mid,i,j,x,y);
    update(node*2+1,mid+1,e,i,j,x,y);
    //ss[node].mn=min(ss[node*2].mn,ss[node*2+1].mn);
    return;
}
...
long long int query(int node, int b, int e, int i, int j)
{
    if(ss[node].!=0)
    {
        ss[node].mn+=(ss[node].pro);
//cout<<"SDS "<<ss[node].pro<<" SDS"<<endl;
        if(e!=b)
        {
            ss[node*2].pro+=(ss[node].pro);
            ss[node*2+1].pro+=(ss[node].pro);
//cout<<"SDS "<<ss[node].mn<<" SDS"<<endl;
        }
        ss[node].pro=0;
    }
...
//cin>>arr[i];
    ...
    FOR(i,0,q)
    {
        ...
        {
            scanf("%lld",&v1);
            ans=query(1,1,num,v1,v1);
           printf("%lld\n",ans);
        }
        else{
        scanf("%Ild%Ild%lld",&v1,&v2,&v3);

//cout<<"SDS"<<ch<<"SDS"<<endl;
                update(1,1,num,v2,v2+v3,v1,v1+v3);
