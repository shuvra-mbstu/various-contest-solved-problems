...
void fun()
{
    for(long long int i=2;i<=Mx;i+=2)
    {
        vt.PB(i*i);
        cout<<i*i<<endl;
    }
...
long long int test, l, r, coun,val1,val2,cas=0;
cin>>test;
while(test--)
{
    cin>>l>>r;
    val2=upper_bound(vt.begin(),vt.end(),r)-vt.begin();
    val1=lower_bound(vt.begin(),vt.end(),l)-vt.begin();
   coun=val2-val1;
   printf("Case %lld: %lld\n",++cas,coun);
}
}
