...
int len=vt.size();
//cout<<vt[len-1]<<endl;
vt.PB(1000000000000000001);
FOR(i,1,len+1){
pre[i]=(pre[i-1]+(((vt[i]-vt[i-1])%MOD)*((vt[i]-vt[i-1])%MOD)))%MOD;
//if(i==len-1)cout<<"SD"<<pre[i]<<endl;
...
long long int num1,num2,a,b,res,test;
fun(0);
...
cin>>num1>>num2;
cout<<num1<<" "<<num2<<endl;
a=lower_bound(vt.begin(),vt.end(),num1)-vt.begin();
b=lower_bound(vt.begin(),vt.end(),num2)-vt.begin();
if(vt[a]>num2){
    cout<<"-1\n";
   continue;
}
