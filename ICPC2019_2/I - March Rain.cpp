...
long long int num,k,arr[100007],test,mx,cost[100007];
long long int t, coun,res, sum,cas=0,vess;
...
bool check(long long int v)
{
    ...
    for(int i=1; i<=num; i++)
    {
     //    cout<<coun<<" "<<tm<<" "<<cost[i]<<endl;

        if(tm+cost[i]<=v)
        {
            tm += cost[i];
      //      cout<<cost[i]<<" "<<v<<" "<<i<<endl;
        }
        else
        {
            cont++;
