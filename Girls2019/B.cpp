...
vector<long long int> vto,vte;
void fibo(){
long long int x=0,sum=0,y=1;
vto.PB(1);
vte.PB(0);
while(sum<=Mx){
     sum=x+y;
...
//    freopen("output.txt", "w", stdout);
   ...
   long long int val,test,aa,bb;
   cin>>test;
   fibo();
   //FOR(i,0,25){
   //cout<<"odd"<<vto[i]<<" even "<<vte[i]<<endl;
   //}
  // cout<<vto.size()<<" "<<vte.size()<<endl;
   while(test--){
   cin>>val;
   aa=upper_bound(vte.begin(),vte.end(),val)-vte.begin();
   bb=upper_bound(vto.begin(),vto.end(),val)-vto.begin();
   cout<<aa<<" "<<bb<<endl;
}
}
