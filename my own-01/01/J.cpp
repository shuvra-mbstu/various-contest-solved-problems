...
int main()
{
 ...
 long long  int arr1[100],arr2[100];
  ...
  sort(arr1,arr1+a);
  sort(arr2,arr2+b);
  long long int res1=-1*Mx,res2;
  int pos=-1;
  for(int j=0;j<b;j++){
  for(int i=0;i<a;i++){

 // cout<<res<<" "<<arr2[j]<<" "<<arr1[i]<<" "<<arr2[j]*arr1[i]<<endl;
  if( res1<((long long int)arr2[j]*arr1[i]) )
  {
      res2=res1;
      res1=arr2[j]*arr1[i];
     //   cout<<res2<<endl;
 ...
 else if((res1==((long long int)arr2[j]*arr1[i])) && pos!=i )
 {
     res2=res1;
     // res1=arr2[j]*arr1[i];
      pos=i;
